...
function Commander (client) {
  ...
  this.actives = {
    ...
    // Background
    flash: (p) => {
      let t = Number(p.str) || 1;
      client.orca.flash(t * 30)
    },
    setsrc: (p) => {
      client.orca.background.src = Number(p.str)
      client.orca.renderBackground()
    },
    resetsrc: (p) => {
      client.orca.background.src = false
      client.orca.renderBackground()
    },
    setfrm: (p) => {
      client.orca.background.frame = Number(p.str)
      client.orca.renderBackground()
    }
  }
