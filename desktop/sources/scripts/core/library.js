...
library.a = function OperatorA (orca, x, y, passive) {
  ...
  this.name = 'array'
  this.info = 'Reads eastward operands'
...
library.b = function OperatorB (orca, x, y, passive) {
  ...
  this.name = 'bigger'
  this.info = 'Outputs biggest input'
  ...
  this.operation = function (force = false) {
    ...
    return a !== '.' && b !== '.' ? orca.keyOf(Math.max(orca.valueOf(a), orca.valueOf(b))) : '.'
  }
}
...
library.m = function OperatorM (orca, x, y, passive) {
  ...
  this.name = 'marker'
  this.info = 'will appear highlighted'
}
...
library.t = function OperatorTrans (orca, x, y, passive) {
  Operator.call(this, orca, x, y, 't', passive)

  this.name = 'transpose'
  this.info = 'transposes the input'

  this.ports.octIn = { x: -2, y: 0 }
  this.ports.noteIn = { x: -1, y: 0 }
  this.ports.octOut = { x: 0, y: 1, output: true }
  this.ports.noteOut = { x: 1, y: 1, output: true }

  this.operation = function (force = false) {
    const n = this.listen(this.ports.noteIn)
    const o = this.listen(this.ports.octIn)
    const scale = "CcDdEeFGgAaB"
    var note = orca.valueOf(n)
    var octave = orca.keyOf(Math.floor(note/ 12) + orca.valueOf(o))
    note %= 12
    note = scale.charAt(note)
    this.output(note, this.ports.noteOut)
    this.output(octave, this.ports.octOut)
  }
}
...
library.z = function OperatorZ (orca, x, y, passive) {
  ...
  this.ports.output = { x: 0, y: 1, default: '.',sensitive: true, reader: true, output: true }

  this.operation = function (force = false) {
    const rate = this.listen(this.ports.rate, true)
    const target = this.listen(this.ports.target, true)
    const val = this.listen(this.ports.output, true)
    const mod = val <= target - 1 ? 1 : val >= target + 1 ? -1 : target - val
    if(target != '.'){
      return orca.f % rate == 0 ? orca.keyOf(val + mod) : orca.keyOf(val)
    }
    else{
      return '.'
    }
...
// MODS by 3000
library['+'] = function OperatorAdd (orca, x, y, passive) {
...
library['-'] = function OperatorSub (orca, x, y, passive) {
  Operator.call(this, orca, x, y, '-', passive)
...
library['*'] = function OperatorDiv (orca, x, y, passive) {
  Operator.call(this, orca, x, y, '*', passive)
...
library['/'] = function OperatorDiv (orca, x, y, passive) {
  ...
  this.name = 'divide'
  this.info = 'Outputs quotient of inputs'
  ...
  this.operation = function (force = false) {
    const a = this.listen(this.ports.a, true)
    const b = this.listen(this.ports.b, true)
    return b == 0 ? orca.keyOf(0) : orca.keyOf(Math.floor(a / b))
  }
}
library['%'] = function OperatorMod (orca, x, y, passive) {
  Operator.call(this, orca, x, y, '%', passive)

  this.name = 'modulo'
  this.info = 'Outputs modulo of inputs'
...
library['='] = function OperatorEq (orca, x, y, passive) {
  Operator.call(this, orca, x, y, '=', passive)
...
library['!'] = function OperatorNot (orca, x, y, passive) {
  Operator.call(this, orca, x, y, '=', passive)

  this.name = 'not'
  this.info = 'Bangs if inputs are not equal'
...
library['('] = function OperatorUpp (orca, x, y, passive) {
  Operator.call(this, orca, x, y, '(', passive)

  this.name = 'uppercase'
  this.info = 'Outputs uppercase of input'
  ...
  this.operation = function (force = false) {
    const a = this.listen(this.ports.a)
    return a !== '.' ? orca.keyOf(orca.valueOf(a)).toUpperCase() : '.'
  }
}
library[')'] = function OperatorUpp (orca, x, y, passive) {
  Operator.call(this, orca, x, y, ')', passive)
  ...
  this.name = 'lowercase'
  this.info = 'Outputs lowercase of input'
  ...
  this.operation = function (force = false) {
    const a = this.listen(this.ports.a)
    return  a !== '.' ? orca.keyOf(orca.valueOf(a)).toLowerCase() : '.'
  }
}
// library['&'] = function OperatorAnd (orca, x, y, passive) {
//   Operator.call(this, orca, x, y, '&', passive)
//
//   this.name = 'and'
//   this.info = 'Bangs if both inputs are bangs'
//
//   this.ports.a = { x: -1, y: 0 }
//   this.ports.b = { x: 1, y: 0 }
//   this.ports.output = { x: 0, y: 1, bang: true, output: true }
...
//
//   this.name = 'or'
//   this.info = 'Bangs if one input is a bang'
//
//   this.ports.a = { x: -1, y: 0 }
//   this.ports.b = { x: 1, y: 0 }
//   this.ports.output = { x: 0, y: 1, bang: true, output: true }
...
//   }
// }
// library['<'] = function OperatorSm (orca, x, y, passive) {
//   Operator.call(this, orca, x, y, '<', passive)
//
//   this.name = 'smaller'
//   this.info = 'Bangs if left is smaller'
//
//   this.ports.a = { x: -1, y: 0 }
//   this.ports.b = { x: 1, y: 0 }
//   this.ports.output = { x: 0, y: 1, bang: true, output: true }
...
//
//     this.name = 'larger'
//     this.info = 'Bangs if left is larger'
//
//     this.ports.a = { x: -1, y: 0 }
//     this.ports.b = { x: 1, y: 0 }
//     this.ports.output = { x: 0, y: 1, bang: true, output: true }
